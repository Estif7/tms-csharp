...
var dataService = new RegistrationDataService();
...
// Trigger independent async I/O fetch tasks concurrently
var studentTasks = studentIds.Select(id => dataService.FetchStudentAsync(id));
var courseTasks = courseCodes.Select(code => dataService.FetchCourseAsync(code));

// Wait non-blockingly for both sets to finish
Student[] students = await Task.WhenAll(studentTasks);
CourseCode[] courses = await Task.WhenAll(courseTasks);
...
try
{
    var overflowCourse = new CourseCode { Code = "CRS-999", Title = "Overflow Test", Capacity = 1, EnrolledCount = 1 };
...
{
    // The calling code now knows exactly which domain context properties caused the violation
